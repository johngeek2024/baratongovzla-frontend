...
import { AdminOrder, OrderStatus, AdminOrderDetail } from '../models/order.model'; // ✅ Importaremos desde un nuevo modelo
...
const mockOrders: AdminOrderDetail[] = [
  ...
  { id: '#BTV-1056', customerName: 'Elena G.', date: '2025-07-09', total: 125.50, status: 'Entregado', items: [], shippingAddress: 'Mañongo, Naguanagua', customerEmail: 'elena.g@email.com' },
...
export class OrderAdminService {
  ...
  updateOrderStatus(orderId: string, status: OrderStatus): Observable<{ success: boolean }> {
    console.log(`Enviando para ACTUALIZAR estado del pedido ${orderId} a ${status}`);
    const order = mockOrders.find(o => o.id === orderId);
    if (order) {
      order.status = status;
    }
